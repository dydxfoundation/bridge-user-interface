...
export const layoutMixins: Record<string, any> = {
  ...
  withOuterBorderClipped: css`
    ...
    --computed-radius: clamp(
      0px,
      var(--bordered-content-outer-container-width) -
        var(--bordered-content-max-width),
    ...
    clip-path: inset(
      calc(-1 * var(--border-width)) round
        calc(var(--computed-radius) + var(--border-width))
    );
